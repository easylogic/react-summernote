...
export function Summernote({value, children, className, containerClassName, opt, ...props}: SummernoteProps) {
    ...
    useEffect(() => {
        ...
        const summernoteElement = getElement(props.id);

        if (!summernoteElement) {
            console.error(`summernote element is not exists. id: ${props.id}`)
            return;
        }
    ...
    return (
        <div className={`react-summernote ${className}`}>
            <div id={`${props.id}-container`} className={`react-summernote-container ${containerClassName}`}>{value || children}</div>
        </div>
    )
}